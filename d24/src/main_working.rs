...
fn run2(input_file: &Path) {
    ...
    let mut swapped_outputs = find_z_anomalies(&output_map, &mut gates, z_len);
    ...
    assert_eq!(x, 28872341726885);
    assert_eq!(y, 28414614475596);
    ...
    // Result
    let mut all_wires = Vec::with_capacity(swapped_outputs.len() * 2);
